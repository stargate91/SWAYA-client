import { toTitleCase, formatFilename } from '@/lib/formatters';
import { ORGANIZER_PAGE_SIZE_OPTIONS } from '@/lib/paginationConstants';

export const PAGE_SIZE_OPTIONS = ORGANIZER_PAGE_SIZE_OPTIONS;
...
const prettifyToken = toTitleCase;
...
export const isOrganizerItemScene = (item) => {
  if (!item) return false;
  const itemType = String(item.type || item.rawType || '').toLowerCase();
  const scanMode = item.scan_mode || item.rawPayload?.scan_mode || '';
  return itemType === 'scene' || scanMode === 'scenes';
};

export const isOrganizerRowScene = (row) => {
  if (!row) return false;
  const isExtra = row.rawType === 'extra' || row.isExtra;
  if (isExtra) {
    return row.parentType === 'scene' || row.rawPayload?.parent_scan_mode === 'scenes';
  }
  return isOrganizerItemScene(row);
};

export const isOrganizerParentCandidateAdult = (item) => {
  if (!item) return false;
  const itemScanMode = item.scan_mode || '';
  return Boolean(item.matches?.some((m) => m.is_adult))
    || isOrganizerItemScene(item)
    || itemScanMode === 'scenes';
};
...
export const mapOrganizerItemRow = (item, t) => ({
  id: `item-${item.id}`,
  itemId: item.id,
  source: item.source_filename || formatFilename(item.filename),
  target: item.target_filename || formatFilename(item.planned_path),
...
export const mapExtraRow = (item, t) => ({
  id: `extra-${item.id}`,
  itemId: item.id,
  source: formatFilename(item.filename),
  target: formatFilename(item.planned_path),
