...
import { useMatchBrowser } from './useMatchBrowser';
import { useMatchResolve } from './useMatchResolve';
import { MEDIA_TYPES, toMetadataMediaType, scanModeToMediaType } from '@/lib/mediaTypes';
...
export default function useMatchModalViewModel({
  ...
  const handleProviderChange = async (nextProviderOrEvent) => {
    ...
    if (nextProvider === 'stashdb' || nextProvider === 'fansdb' || scanMode === 'scenes') {
      nextMode = 'scene';
      setMode('scene');
    } else if (nextProvider === 'theporndb') {
      nextMode = 'movie';
      setMode('movie');
    ...
    if (hasSearched && !isSearching) {
      ...
      if (searchResults && searchResults.length === 1 && nextMode === MEDIA_TYPES.TV && !['theporndb', 'stashdb', 'fansdb'].includes(nextProvider)) {
        const parsedSeason = Number.parseInt(season, 10);
        if (Number.isFinite(parsedSeason)) {
          handleDirectBrowse(searchResults[0], parsedSeason);
        } else {
  ...
  const handleCandidateSelect = async (candidate) => {
    const mediaType = toMetadataMediaType(candidate.type || candidate.media_type) || scanModeToMediaType(mode) || mode;
